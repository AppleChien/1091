...
int main()
{
   ...
   ifstream inFile( "D:\\school_hw\\Big1.1\\程式設計\\MyHW\\20201028\\Test cases.txt", ios::in );
   ...
   ofstream outFile( "D:\\school_hw\\Big1.1\\程式設計\\MyHW\\20201028\\Result.txt", ios::out );
               ...
               HugeInt &quotient, HugeInt &remainder )
{
    ...
    if (equal(dividend, divisor))   //如果被除式等於除式
    {
        quotient.elems[0] = 1;
        reset(remainder);
        return;
    ...
    int k;
    //dividend < divisor
    if (less(dividend, divisor)) //如果被除式小於除式
    {
        reset(quotient);  
    }
    else
